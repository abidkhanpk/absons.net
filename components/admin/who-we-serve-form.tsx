...
export function WhoWeServeForm({ item }: { item?: WhoWeServeItem }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/admin/who-we-serve", {
        method: item ? "PUT" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(item ? { id: item.id, ...formData } : formData),
      })
      const result = await response.json().catch(() => ({}))
      if (!response.ok) throw new Error(result.error || "Failed to save entry")
      router.push("/admin/who-we-serve")
      router.refresh()
    } catch (error) {
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <div className="flex gap-3 pt-4">
            <Button type="submit" disabled={isSubmitting}>
              {isSubmitting ? "Saving..." : item ? "Update Entry" : "Create Entry"}
            </Button>
